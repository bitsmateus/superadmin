...
type ClientRow = { company: string | null; name: string; stage: string; briefing_status: string | null; delivery_date: string | null };
...
async function buildDigest(): Promise<string> {
  ...
  const clients = await query<ClientRow>(
    `SELECT company, name, stage, briefing_status, delivery_date
     FROM clients
     WHERE stage <> 'churned'`
  );
  ...
  if (!overdue.length && !today.length && !week.length && !pipeline.length) {
...
export function startDailyDigest(): void {
  ...
  console.log('[digest] agendador diário ativo (07h America/Sao_Paulo)');
}
